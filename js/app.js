...
// ═══════════════════════════════════════════
// 6. 自动保存（串行化由 store._saveChain 保证）
// ═══════════════════════════════════════════
...
window.addEventListener('beforeunload', () => {
  ...
  if (c === lastSnap) return;
  // 入队持久化，下次打开页面时自动重放。
  // 注意：不带 version（enqueueOp 也会剥离），重放 = 强制写入，绝不因版本过期被拒。
  enqueueOp({
    type: 'update',
    noteId: f.id,
    payload: { content: c },
...
// 14. 初始化
// ═══════════════════════════════════════════
// 注意：flushQueue 必须在 restore 完成之后执行。
// 若在数据加载前重放，冲突处理找不到目标文件，无法补救。
store.restore().then(async (ok) => {
  // restore 失败（网络抖动/域名错误）时绝不自动新建笔记，
  // 否则每次打开页面都会向服务端多塞一个"未命名 1"。
  if (!ok) {
    toast('获取笔记失败：已跳过自动新建，网络恢复后刷新页面即可');
    return;
  }
  // 只有确认账号确实为空（首次使用）才创建初始笔记
  if (store.files.length === 0) await store.createFile(true);
  renderList();
  await loadEditor();
  applyFontSize(store.fontSize);
  editor.focus();
  flushQueue(); // 数据就绪后再重放离线队列
